...
#		tallies = [{'numberThatAreGreater' : 0, 'leastGreater' : None }] * size
    ...
    def _idx_of_lesser(float x, float y):
		return ((1 + cmp(x,y)/2)

    def count_unique_info(self):
        '''
        In all that follows, there are many lists of size 2.
        The value at position 0 is the index of a lesser value.
        The value at position 1 is the index of a greater value.
        '''
        # Every element of catalog is a two-item list.
        # The first item is the index of the greatest known element of self
        # whose value is less than the current element in self.
        #
        # The second item is the index of the least known element of self
        # whose value is greater than the current element in self.
        #
        # In other words,
        # self[catalog[idx][0]] < self[idx] < self[catalog[idx][1]]
        list = self.list
        N = len(list)
        catalog = [None, None] * N
        info_count = 0
        for j in xrange(1,N):
            ...
            for a in [0,1]:
                ...
                conidx = cmps[b]
                while True:
                    if catalog[idx][b] == None:
                        catalog[idx][b] = conidx
                        break
                    else:
                        idx_of_lesser = _idx_of_lesser(list[catalog[idx][b]], list[catalog[conidx][b]])
                        idx_of_greater = 1 - idx_of_lesser
